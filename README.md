# Microservices using spring boot, spring cloud,zuul gateway & eureka

This project shows how to create microservices with 
...
- We have three independent microservices (User, Site, Organization), which registers themselves in service discovery (Eureka Server),
  and communicate with each other by declarative REST client (Open Feign).
- The whole system is hidden behind API gateway (Zuul).
...
1. **Clone the application**
	...
	```bash
	git clone https://github.com/pavanuppuluri/Microservices-spring-boot-spring-cloud-zuul-gateway-eureka.git
	```

2. **Change the server's port for the applications as per you want**
